...
def proces_thresh(thresh_file, raw_files):

    data = pd.read_pickle(thresh_file)
    files_temp = [file.split('/')[-1] for file in raw_files]
    ...
    for x, file_row in file_df.iterrows():
        ...
        # find file location
        file_loc = raw_files[files_temp.index(file_row.files)]
        # read file info
        ...
        for multipl in data_temp.multipl.unique():
            ...
            print('Processing ' + file_row.files)
